...
def get_link_status_page_html(uid: str) -> str:
    """صفحه‌ی وضعیت یک کانفیگ تکی — همون /sub/{uid} وقتی توی مرورگر باز بشه (نه توی اپ VPN)."""
    return f"""<!DOCTYPE html>
<html lang="fa" dir="rtl">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0">
<title>X4G Config</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@tabler/icons-webfont@3.19.0/dist/tabler-icons.min.css">
<style>
...
.bg-fx{{position:fixed;inset:0;background:radial-gradient(ellipse 70% 45% at 50% -8%,rgba(59,124,246,0.13),transparent 62%),var(--bg);z-index:0;pointer-events:none;transition:background .35s}}
.grid-fx{{position:fixed;inset:0;background-image:linear-gradient(rgba(96,148,246,0.025) 1px,transparent 1px),linear-gradient(90deg,rgba(96,148,246,0.025) 1px,transparent 1px);background-size:46px 46px;z-index:0;pointer-events:none}}
.wrap{{position:relative;z-index:10;max-width:640px;margin:0 auto;padding:24px 16px 64px}}
.top{{display:flex;align-items:center;justify-content:space-between;margin-bottom:26px;gap:10px}}
.brand{{display:flex;align-items:center;gap:11px;min-width:0}}
.brand-img{{width:40px;height:40px;border-radius:50%;overflow:hidden;border:1px solid var(--card-b);box-shadow:0 0 14px rgba(139,92,246,.3),0 0 8px rgba(59,130,246,.25);flex-shrink:0}}
.brand-img img{{width:100%;height:100%;object-fit:cover}}
...
.sub-info{{background:var(--card);border:1px solid var(--card-b);border-radius:22px;padding:26px 24px 24px;margin-bottom:16px;box-shadow:var(--shadow);position:relative;overflow:hidden}}
.sub-info::before{{content:'';position:absolute;top:0;right:0;width:180px;height:180px;background:radial-gradient(circle at top right,rgba(59,124,246,.12),transparent 70%);pointer-events:none}}
.sub-eyebrow{{font-size:10px;font-weight:700;color:var(--accent2);text-transform:uppercase;letter-spacing:.12em;margin-bottom:9px;display:flex;align-items:center;gap:6px}}
.sub-eyebrow i{{font-size:13px}}
.sub-name{{font-size:24px;font-weight:800;color:var(--t1);margin-bottom:10px;letter-spacing:-.02em;display:flex;align-items:center;gap:10px;flex-wrap:wrap}}
.cfg-status{{display:flex;align-items:center;gap:5px;font-size:11px;font-weight:700;padding:5px 12px;border-radius:20px;white-space:nowrap}}
.cfg-status.ok{{background:var(--green-bg);color:var(--green-t)}}
.cfg-status.no{{background:var(--red-bg);color:var(--red-t)}}
.proto-chip{{font-size:10px;padding:4px 10px;border-radius:8px;font-weight:800;letter-spacing:.02em}}
.pc-ws{{background:var(--accent-d);color:var(--accent2)}}
.pc-xhttp{{background:var(--purple-bg);color:var(--purple-t)}}
.pc-ultra{{background:var(--green-bg);color:var(--green-t)}}
.sub-meta-row{{font-size:11px;color:var(--t3);margin-bottom:16px;display:flex;align-items:center;gap:14px;flex-wrap:wrap}}
.sub-meta-row span{{display:flex;align-items:center;gap:5px}}

.usage-block{{margin-bottom:6px}}
.ubar{{height:9px;border-radius:6px;background:rgba(96,148,246,0.1);overflow:hidden;margin-bottom:8px}}
.ubar-f{{height:100%;border-radius:6px;transition:width .5s ease}}
.utxt{{font-size:11.5px;color:var(--t2);display:flex;justify-content:space-between;font-weight:600}}

.stats-bar{{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:18px}}
.stat-card{{background:var(--card);border:1px solid var(--card-b);border-radius:16px;padding:16px 15px;transition:.2s}}
.stat-card:hover{{border-color:var(--card-bh);transform:translateY(-1px)}}
.stat-label{{font-size:9px;color:var(--t3);font-weight:700;text-transform:uppercase;letter-spacing:.06em;margin-bottom:7px}}
.stat-val{{font-size:18px;font-weight:800;color:var(--t1);line-height:1;letter-spacing:-.01em}}
.stat-sub{{font-size:9.5px;color:var(--t3);margin-top:6px}}

.cfg-title{{font-size:12px;font-weight:800;color:var(--t2);margin-bottom:13px;display:flex;align-items:center;gap:6px;text-transform:uppercase;letter-spacing:.07em}}
.cfg-title i{{color:var(--accent);font-size:15px}}

.cfg-card{{background:var(--card);border:1px solid var(--card-b);border-radius:18px;transition:all .2s;position:relative;overflow:hidden;margin-bottom:16px}}
.cfg-bottom{{padding:18px 19px 19px}}
.cfg-link-toggle{{width:100%;display:flex;align-items:center;justify-content:space-between;gap:10px;background:transparent;border:1px dashed var(--card-b);border-radius:11px;padding:12px 14px;cursor:pointer;font-family:inherit;color:var(--t2);font-size:12px;font-weight:600;transition:.15s}}
.cfg-link-toggle:hover{{border-color:var(--card-bh);color:var(--t1)}}
.cfg-link-toggle i.ti-chevron-down{{transition:transform .2s}}
.cfg-link-toggle.open i.ti-chevron-down{{transform:rotate(180deg)}}
.ltl{{display:flex;align-items:center;gap:7px}}
.cfg-vless-wrap{{max-height:0;overflow:hidden;transition:max-height .25s ease}}
.cfg-vless-wrap.open{{max-height:200px;margin-top:10px}}
.cfg-vless-inner{{background:rgba(0,0,0,.22);border-radius:11px;padding:12px 13px}}
[data-theme="light"] .cfg-vless-inner{{background:rgba(46,99,214,.05)}}
.cfg-vless{{font-family:ui-monospace,monospace;font-size:10.5px;color:var(--t2);word-break:break-all;line-height:1.7;max-height:110px;overflow-y:auto}}
.cfg-actions{{display:flex;gap:9px;margin-top:13px;flex-wrap:wrap}}
.btn{{display:inline-flex;align-items:center;gap:6px;padding:9px 16px;border-radius:11px;border:none;font-family:inherit;font-size:12px;font-weight:700;cursor:pointer;transition:.15s}}
.btn-p{{background:var(--accent);color:#fff}}
.btn-p:hover{{background:var(--accent2)}}
.btn-g{{background:var(--green-bg);color:var(--green-t)}}
.btn-g:hover{{background:rgba(31,184,126,.22)}}
.btn-pur{{background:var(--purple-bg);color:var(--purple-t)}}
.btn-pur:hover{{background:rgba(157,123,240,.22)}}
.conn-chip{{display:inline-flex;align-items:center;gap:4px;font-size:10px;padding:4px 10px;border-radius:20px;background:var(--green-bg);color:var(--green-t);font-weight:700}}
.dot{{width:5px;height:5px;border-radius:50%;background:var(--green);display:inline-block;animation:pulse 2s infinite}}
@keyframes pulse{{0%,100%{{opacity:1}}50%{{opacity:.25}}}}

.empty-state{{text-align:center;padding:80px 20px;color:var(--t3)}}
.empty-state i{{font-size:38px;display:block;margin-bottom:14px}}
.empty-state.err{{color:var(--red-t)}}
...
<div class="wrap">
  <div class="top">
    <div class="brand">
      <div class="brand-img"><img src="data:image/png;base64,{LOGO_B64}" alt="X4G"></div>
      <div><div class="brand-name">X4G</div><div class="brand-sub">وضعیت کانفیگ</div></div>
    </div>
    <div class="top-actions">
      <button class="icon-btn" id="theme-toggle" onclick="toggleTheme()" title="تغییر تم"><i class="ti ti-sun" id="theme-icon"></i></button>
    </div>
...
function toggleLink(){{
  const wrap=document.getElementById('vw-0');
  const btn=document.getElementById('vt-0');
  const open=wrap.classList.toggle('open');
  btn.classList.toggle('open',open);
  btn.querySelector('.ltl span').textContent = open ? 'پنهان کردن لینک' : 'نمایش لینک کانفیگ';
}}
...
async function loadData(){{
  const r=await fetch('/api/public/link/'+UID);
...
function renderNotFound(){{
  document.getElementById('root').innerHTML =
    '<div class="empty-state err"><i class="ti ti-alert-circle"></i>این کانفیگ پیدا نشد یا حذف شده.</div>';
}}

function renderContent(l){{
  window._x4gVless = l.vless_link;
  window._x4gSub   = l.sub_url;
  window._x4gLabel = l.label;

  const pct = l.limit_bytes === 0 ? 0 : Math.min(100, l.used_bytes / l.limit_bytes * 100);
  const bc  = pct > 90 ? 'var(--red)' : pct > 70 ? 'var(--amber)' : 'var(--green)';
  const lim = l.limit_bytes === 0 ? '∞' : l.limit_fmt;
  const dl  = daysLeft(l.expires_at);
  let expTxt = 'بدون انقضا';
  if (dl !== null) expTxt = dl > 0 ? `${{toFa(dl)}} روز باقی‌مانده` : 'منقضی شده';

  document.getElementById('root').innerHTML=`
    <div class="sub-info">
      <div class="sub-eyebrow"><i class="ti ti-router"></i> کانفیگ VPN</div>
      <div class="sub-name">${{esc(l.label)}} <span class="cfg-status ${{l.active ? 'ok' : 'no'}}">${{l.active ? '<i class="ti ti-circle-check"></i> فعال' : '<i class="ti ti-circle-x"></i> غیرفعال'}}</span></div>
      <div class="sub-meta-row">
        ${{protoChip(l.protocol)}}
        <span><i class="ti ti-calendar-time"></i> ${{expTxt}}</span>
        ${{l.connections > 0 ? `<span class="conn-chip"><span class="dot"></span> ${{toFa(l.connections)}} اتصال زنده</span>` : ''}}
      </div>
      <div class="usage-block">
        <div class="ubar"><div class="ubar-f" style="width:${{pct}}%;background:${{bc}}"></div></div>
        <div class="utxt"><span>${{esc(l.used_fmt)}} مصرف شده</span><span>سهمیه: ${{esc(lim)}}</span></div>
      </div>
    </div>
    ...
    <div class="stats-bar">
      <div class="stat-card">
        ...
        <div class="stat-val" style="font-size:15px">${{esc(l.used_fmt)}}</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">سهمیه کل</div>
        <div class="stat-val" style="font-size:15px">${{esc(lim)}}</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">اتصالات زنده</div>
        <div class="stat-val">${{toFa(l.connections)}}</div>
      </div>
    </div>

    <div class="cfg-title"><i class="ti ti-link"></i> لینک اتصال</div>
    <div class="cfg-card">
      <div class="cfg-bottom">
        <button class="cfg-link-toggle" id="vt-0" onclick="toggleLink()">
          <span class="ltl"><i class="ti ti-eye"></i> <span>نمایش لینک کانفیگ</span></span>
          <i class="ti ti-chevron-down"></i>
        </button>
        <div class="cfg-vless-wrap" id="vw-0">
          <div class="cfg-vless-inner">
            <div class="cfg-vless">${{esc(l.vless_link)}}</div>
          </div>
        </div>
        <div class="cfg-actions">
          <button class="btn btn-p" onclick="navigator.clipboard.writeText(window._x4gVless).then(()=>toast('لینک کانفیگ کپی شد ✓','ok'))">
            <i class="ti ti-copy"></i> کپی لینک کانفیگ
          </button>
          <button class="btn btn-g" onclick="showQR(window._x4gLabel, window._x4gVless)">
            <i class="ti ti-qrcode"></i> QR
          </button>
          <button class="btn btn-pur" onclick="navigator.clipboard.writeText(window._x4gSub).then(()=>toast('لینک ساب کپی شد ✓','ok'))">
            <i class="ti ti-link"></i> کپی لینک ساب
          </button>
        </div>
      </div>
    </div>
