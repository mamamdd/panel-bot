...
# ══════════════════════════════════════════════════════════════════════════════
# فروشگاه (خرید کانفیگ با کارت‌به‌کارت + تایید دستی ادمین)
# ══════════════════════════════════════════════════════════════════════════════
# اطلاعات کارت برای واریز مشتری‌ها؛ از متغیر محیطی می‌گیره تا شماره کارت داخل کد نباشه.
CARD_NUMBER = os.environ.get("X4G_CARD_NUMBER", "").strip()
CARD_HOLDER = os.environ.get("X4G_CARD_HOLDER", "").strip()
CURRENCY_LABEL = os.environ.get("X4G_CURRENCY_LABEL", "تومان").strip()

# پلن‌های فروش — هرجور خواستی این لیست رو ویرایش کن (قیمت، حجم، روز اعتبار).
# id هر پلن باید یکتا و بدون فاصله باشه.
PLANS = [
    {"id": "p1", "title": "۱۰ گیگابایت / ۳۰ روز", "price": 50000, "volume_gb": 10, "days": 30, "speed_mbps": 0},
    {"id": "p2", "title": "۳۰ گیگابایت / ۳۰ روز", "price": 120000, "volume_gb": 30, "days": 30, "speed_mbps": 0},
    {"id": "p3", "title": "۱۰۰ گیگابایت / ۳۰ روز", "price": 300000, "volume_gb": 100, "days": 30, "speed_mbps": 0},
    {"id": "p4", "title": "نامحدود / ۳۰ روز", "price": 450000, "volume_gb": 0, "days": 30, "speed_mbps": 0},
]

def _plan_by_id(pid: str) -> dict | None:
    return next((p for p in PLANS if p["id"] == pid), None)

ORDERS_FILE = DATA_DIR / "x4g_orders.json"
    ...
    try:
        if ORDERS_FILE.exists():
            async with aiofiles.open(ORDERS_FILE, "r", encoding="utf-8") as f:
                raw = await f.read()
            ORDERS.update(json.loads(raw) or {})
...
async def _save_orders():
    async with ORDERS_LOCK:
        try:
            ...
            logger.warning(f"Could not save orders: {e}")
...
# ── Store (customer-facing) keyboards ─────────────────────────────────────────
def _customer_menu_kb():
    return {"inline_keyboard": [
        [{"text": "🛒 خرید کانفیگ", "callback_data": "buy:list"}],
    ]}

def _plans_kb():
    rows = []
    for p in PLANS:
        rows.append([{"text": f"{p['title']} — {p['price']:,} {CURRENCY_LABEL}", "callback_data": f"buyplan:{p['id']}"}])
    rows.append([{"text": "❌ انصراف", "callback_data": "buy:cancel"}])
    return {"inline_keyboard": rows}

def _buy_cancel_kb():
    return {"inline_keyboard": [[{"text": "❌ انصراف خرید", "callback_data": "buy:cancel"}]]}

def _order_admin_kb(oid: str):
    return {"inline_keyboard": [
        [{"text": "✅ تایید و صدور کانفیگ", "callback_data": f"orderok:{oid}"},
         {"text": "⛔ رد کردن", "callback_data": f"orderno:{oid}"}],
    ]}

def _format_plan(p: dict) -> str:
    vol = "نامحدود" if not p.get("volume_gb") else f"{p['volume_gb']} گیگابایت"
    speed = "نامحدود" if not p.get("speed_mbps") else f"{p['speed_mbps']} Mbps"
    return (
        f"📦 <b>{p['title']}</b>\n"
        f"حجم: {vol}\n"
        f"اعتبار: {p['days']} روز\n"
        f"سرعت: {speed}\n"
        f"قیمت: <b>{p['price']:,} {CURRENCY_LABEL}</b>"
    )
...
# ── Store (customer-facing) handlers ──────────────────────────────────────────
async def _notify_admins_new_order(order: dict):
    plan = _plan_by_id(order["plan_id"]) or {}
    caption = (
        "🧾 سفارش جدید در انتظار تایید\n\n"
        f"مشتری: {order.get('first_name','')} (@{order.get('username') or '—'})\n"
        f"آیدی عددی: <code>{order['user_id']}</code>\n"
        f"پلن: {plan.get('title','?')} — {plan.get('price',0):,} {CURRENCY_LABEL}\n"
        f"شماره سفارش: <code>{order['id']}</code>\n\n"
        "رسید پرداخت رو بررسی کن و تایید یا رد کن:"
    )
    kb = _order_admin_kb(order["id"])
    sent = []
    for admin_id in ADMIN_IDS:
        res = await _call(
            "sendPhoto", chat_id=admin_id, photo=order["receipt_file_id"],
            caption=caption, parse_mode="HTML", reply_markup=kb,
        )
        if res and res.get("ok"):
            sent.append({"chat_id": admin_id, "message_id": res["result"]["message_id"]})
    order["admin_msgs"] = sent

async def _handle_customer_message(chat_id: int, msg: dict, text: str):
    if text in ("/start", "/menu"):
        _pending.pop(chat_id, None)
        await _send(chat_id, "👋 به فروشگاه X4G خوش اومدی.\nبرای خرید کانفیگ روی دکمه‌ی زیر بزن:", _customer_menu_kb())
        return

    if text == "/cancel":
        _pending.pop(chat_id, None)
        await _send(chat_id, "لغو شد.", _customer_menu_kb())
        return

    pending = _pending.get(chat_id)
    if pending and pending.get("action") == "await_receipt":
        oid = pending["order_id"]
        order = ORDERS.get(oid)
        if not order or order.get("status") != "await_receipt":
            _pending.pop(chat_id, None)
            await _send(chat_id, "این سفارش دیگه معتبر نیست. دوباره از منو شروع کن:", _customer_menu_kb())
            return
        photo = msg.get("photo")
        if not photo:
            await _send(chat_id, "❗️ لطفاً عکس رسید واریز رو ارسال کن (نه متن).", _buy_cancel_kb())
            return
        ...
        _pending.pop(chat_id, None)
        await _save_orders()
        await _notify_admins_new_order(order)
        await _save_orders()
        await _send(chat_id, "✅ رسید دریافت شد و برای ادمین ارسال شد.\nبه محض تایید، لینک کانفیگ برات ارسال می‌شه.", _customer_menu_kb())
        return

    # پیام ناشناخته → منوی فروشگاه رو نشون بده
    await _send(chat_id, "از دکمه‌ی زیر برای خرید استفاده کن:", _customer_menu_kb())

async def _handle_customer_callback(chat_id: int, message_id: int, data: str):
    if data in ("buy:list", "buy:cancel") or data == "menu":
        _pending.pop(chat_id, None)
        if data == "buy:cancel" or data == "menu":
            await _edit(chat_id, message_id, "👋 به فروشگاه X4G خوش اومدی.\nبرای خرید کانفیگ روی دکمه‌ی زیر بزن:", _customer_menu_kb())
            return
        await _edit(chat_id, message_id, "یکی از پلن‌های زیر رو انتخاب کن:", _plans_kb())
        return

    if data.startswith("buyplan:"):
        pid = data.split(":", 1)[1]
        plan = _plan_by_id(pid)
        if not plan:
            await _edit(chat_id, message_id, "این پلن دیگه موجود نیست.", _plans_kb())
            return
        if not CARD_NUMBER:
            await _edit(chat_id, message_id, "⚠️ فروشگاه هنوز فعال نشده (شماره کارت تنظیم نشده). به ادمین اطلاع بده.", _customer_menu_kb())
            return
        ...
        ORDERS[oid] = order
        await _save_orders()
        _pending[chat_id] = {"action": "await_receipt", "order_id": oid}
        card_txt = f"<code>{CARD_NUMBER}</code>" + (f"\nبه نام: {CARD_HOLDER}" if CARD_HOLDER else "")
        await _edit(
            chat_id, message_id,
            f"{_format_plan(plan)}\n\n"
            f"💳 مبلغ رو به شماره کارت زیر واریز کن:\n{card_txt}\n\n"
            "بعد از واریز، عکس رسید رو همینجا ارسال کن تا برای ادمین بره.",
            _buy_cancel_kb(),
        )
        return

    await _answer_cb_noop()

async def _answer_cb_noop():
    return

async def _handle_order_decision(admin_chat_id: int, message_id: int, data: str):
    action, oid = data.split(":", 1)
    order = ORDERS.get(oid)
    if not order:
        await _edit(admin_chat_id, message_id, "این سفارش دیگه پیدا نشد.")
        return
    if order.get("status") != "pending_review":
        await _edit(admin_chat_id, message_id, f"این سفارش قبلاً بررسی شده (وضعیت: {order.get('status')}).")
        return
    ...
    if action == "orderok":
        ...
        limit_bytes = 0 if not volume_gb else parse_size_to_bytes(volume_gb, "GB")
        days = plan.get("days", 30)
        expires_at = (datetime.now() + timedelta(days=days)).isoformat() if days > 0 else None
        speed_mbps = plan.get("speed_mbps", 0)
        speed_limit_bytes = 0 if not speed_mbps else parse_speed_to_bytes(speed_mbps, "MBIT")
        uid, link = await make_link(
            label=f"مشتری {buyer_id} — {plan.get('title','')}",
            limit_bytes=limit_bytes,
            expires_at=expires_at,
            speed_limit_bytes=speed_limit_bytes,
            note=f"فروش تلگرام — سفارش {oid}",
        )
        ...
        await _send(
            buyer_id,
            f"✅ پرداخت تایید شد و کانفیگت آماده‌ست:\n\n"
            f"🔗 لینک اتصال:\n<code>{vless}</code>\n\n"
            f"🔗 لینک ساب:\n<code>{sub_url}</code>",
        )
        for m in order.get("admin_msgs", []):
            await _edit(m["chat_id"], m["message_id"], f"✅ سفارش {oid} تایید و کانفیگ صادر شد.")
    else:
        order["status"] = "rejected"
        await _save_orders()
        await _send(buyer_id, "⛔ رسید پرداختت تایید نشد. اگه فکر می‌کنی اشتباهیه، با پشتیبانی در تماس باش.")
        for m in order.get("admin_msgs", []):
            await _edit(m["chat_id"], m["message_id"], f"⛔ سفارش {oid} رد شد.")
...
async def _handle_message(msg: dict):
    ...
    if not _is_admin(chat_id):
        ...
        # این نگه‌داری فقط برای نمایش نام/یوزرنیم مشتری توی پیام ادمینه
        for oid, o in ORDERS.items():
            if o.get("user_id") == chat_id and o.get("status") in ("await_receipt",):
                o["username"] = from_user.get("username")
                o["first_name"] = from_user.get("first_name", "")
        await _handle_customer_message(chat_id, msg, text)
        return
...
async def _handle_callback(cb: dict):
    ...
    if not _is_admin(chat_id):
        ...
        await _handle_customer_callback(chat_id, message_id, data)
        return

    await _answer_cb(cb_id)

    if data.startswith("orderok:") or data.startswith("orderno:"):
        await _handle_order_decision(chat_id, message_id, data)
        return
...
async def start_bot():
    ...
    if not CARD_NUMBER:
        logger.info("Telegram bot: X4G_CARD_NUMBER تنظیم نشده، بخش فروشگاه برای مشتری‌ها غیرفعاله.")
    await _load_orders()
